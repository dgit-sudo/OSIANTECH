...
const sessionAccessSecret = process.env.SESSION_ACCESS_SECRET;
if (!instructorTokenSecret || !sessionAccessSecret) {
  throw new Error('INSTRUCTOR_AUTH_SECRET and SESSION_ACCESS_SECRET must be set');
}
...
async function createMeetingFromActivation(activation) {
  ...
  const lockKey = `${activation.uid}:${activation.courseId}:${activation.classNo}`;
  ...
  try {
    ...
    if (existing.rows[0]?.meeting_id) {
      selectedMeetingId = String(existing.rows[0].meeting_id);
      await client.query('commit');
    } else {
      let meetingId = '';
      for (let attempt = 0; attempt < 5; attempt += 1) {
        meetingId = crypto.randomBytes(8).toString('hex');
        const check = await client.query(`select 1 from ${liveSessionsTable} where meeting_id = $1 limit 1`, [meetingId]);
  ...
  const meeting = await getMeetingById(selectedMeetingId);
...
async function forceEndMeeting(meetingId) {
  await ensureSessionTables();
  const meeting = await getMeetingById(meetingId);
  if (!meeting) throw new Error('Meeting not found.');

  await pool.query(
    `delete from ${liveSessionsTable} where meeting_id = $1`,
    [meetingId],
  );
